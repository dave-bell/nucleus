<!-- Context: project-intelligence/decisions | Priority: high | Version: 1.32 | Updated: 2026-08-28 -->
...
| 29 | Data Export inline edit — `Nucleus.NomadVars.update/5`, not the issue's own `update/4`: `items` must be a parameter since `Store.write/2` replaces the whole map and a fresh `Store.read/0` would defeat CAS's purpose; `update/5` also validates `value` via `Value.validate/1` before the CAS check, wrapping the bare reason atom into `Error{kind: :invalid}` (caught in review — the first pass trusted the LiveView's own changeset as the only gate); row-scoped form swap inside `#var-{key}-value` (no modal — `DEX-A03` values are unmasked, nothing to reveal-gate); `save_edit` re-checks `editing_key` against the submitted key by pattern match, mirroring `SecretsLive`; `:conflict` gets its own "reload and retry" copy, every other kind collapses to one generic message; `env_names` rejected inside `handle_event("edit", ...)` itself, not only by omitting its button | 2026-08-28 | Decided | `docs/adr/0029-data-export-inline-edit-update-arity-and-conflict-copy.md` |
...
- [ ] Read the Decision Index above; `adr/0001`–`0029` are binding
