...
This file lists external libraries/frameworks that should use **external-lookup** (via Context7) for live documentation instead of relying on potentially outdated training data.

## When to Use This

**context-indexer** checks this list when:
1. User asks about a library/framework
2. No internal context exists in `{context_root}/development/frameworks/ (if present)`
3. Query matches a library name below

**Action**: Recommend **external-lookup** agent
...
context-indexer and external-lookup should match queries containing:
...
4. Update external-lookup if needed (usually automatic)
...
## Usage by external-lookup

external-lookup uses this file to:
